...
void create_ui(display_ui_t *ui)
{
    ...
    lv_obj_set_size(announcement_container, 220, 90); // 设置尺寸
    lv_obj_align(announcement_container, LV_ALIGN_LEFT_MID, 10, -120); // 左中对齐，Y轴偏移-120
    lv_obj_set_style_bg_color(announcement_container, lv_color_hex(0x1a1a2e), 0); // 深蓝色背景
    lv_obj_set_style_bg_opa(announcement_container, LV_OPA_80, 0); // 80%不透明度
    ...
    lv_obj_set_size(discount_container, 220, 70); // 设置尺寸
    lv_obj_align(discount_container, LV_ALIGN_LEFT_MID, 10, 0); // 左中对齐，无Y偏移
    lv_obj_set_style_bg_color(discount_container, lv_color_hex(0x1a1a2e), 0); // 深蓝色背景
    lv_obj_set_style_bg_opa(discount_container, LV_OPA_80, 0); // 80%不透明度
    ...
    lv_obj_set_size(gift_container, 220, 70); // 设置尺寸
    lv_obj_align(gift_container, LV_ALIGN_LEFT_MID, 10, 90); // 左中对齐，Y轴偏移90
    lv_obj_set_style_bg_color(gift_container, lv_color_hex(0x1a1a2e), 0); // 深蓝色背景
    lv_obj_set_style_bg_opa(gift_container, LV_OPA_80, 0); // 80%不透明度
    ...
    // ==================== 左右切换箭头按钮 ====================
    // 左箭头按钮 - 用于切换到上一辆汽车
    ui->prev_btn = lv_imgbtn_create(ui->screen);
    // 设置按钮在不同状态下的图片源（使用left图片资源）
    lv_imgbtn_set_src(ui->prev_btn, LV_IMGBTN_STATE_RELEASED, &left, NULL, NULL);
    lv_obj_set_size(ui->prev_btn, 60, 60); // 按钮尺寸：60x60像素
    lv_obj_align(ui->prev_btn, LV_ALIGN_BOTTOM_LEFT, 20, -20); // 对齐到左下角
    // 设置各种状态下的透明度为0，使按钮背景完全透明
    lv_obj_set_style_bg_opa(ui->prev_btn, LV_OPA_0, 0);
    lv_obj_set_style_border_opa(ui->prev_btn, LV_OPA_0, 0);
    lv_obj_set_style_outline_opa(ui->prev_btn, LV_OPA_0, 0);
    lv_obj_set_style_shadow_opa(ui->prev_btn, LV_OPA_0, 0);
    // 按下状态下的透明度设置
    lv_obj_set_style_bg_opa(ui->prev_btn, LV_OPA_0, LV_STATE_PRESSED);
    lv_obj_set_style_border_opa(ui->prev_btn, LV_OPA_0, LV_STATE_PRESSED);
    lv_obj_set_style_outline_opa(ui->prev_btn, LV_OPA_0, LV_STATE_PRESSED);
    // 聚焦状态下的透明度设置
    lv_obj_set_style_bg_opa(ui->prev_btn, LV_OPA_0, LV_STATE_FOCUSED);
    lv_obj_set_style_border_opa(ui->prev_btn, LV_OPA_0, LV_STATE_FOCUSED);
    lv_obj_set_style_outline_opa(ui->prev_btn, LV_OPA_0, LV_STATE_FOCUSED);
    // 添加点击事件回调函数
    lv_obj_add_event_cb(ui->prev_btn, prev_car_callback, LV_EVENT_CLICKED, NULL);

    // 右箭头按钮 - 用于切换到下一辆汽车
    ui->next_btn = lv_imgbtn_create(ui->screen);
    // 设置按钮在不同状态下的图片源（使用right图片资源）
    lv_imgbtn_set_src(ui->next_btn, LV_IMGBTN_STATE_RELEASED, &right, NULL, NULL);
    lv_obj_set_size(ui->next_btn, 60, 60); // 按钮尺寸：60x60像素
    lv_obj_align(ui->next_btn, LV_ALIGN_BOTTOM_RIGHT, -20, -20); // 对齐到右下角
    // 设置各种状态下的透明度为0，使按钮背景完全透明
    lv_obj_set_style_bg_opa(ui->next_btn, LV_OPA_0, 0);
    lv_obj_set_style_border_opa(ui->next_btn, LV_OPA_0, 0);
    lv_obj_set_style_outline_opa(ui->next_btn, LV_OPA_0, 0);
    lv_obj_set_style_shadow_opa(ui->next_btn, LV_OPA_0, 0);
    // 按下状态下的透明度设置
    lv_obj_set_style_bg_opa(ui->next_btn, LV_OPA_0, LV_STATE_PRESSED);
    lv_obj_set_style_border_opa(ui->next_btn, LV_OPA_0, LV_STATE_PRESSED);
    lv_obj_set_style_outline_opa(ui->next_btn, LV_OPA_0, LV_STATE_PRESSED);
    // 聚焦状态下的透明度设置
    lv_obj_set_style_bg_opa(ui->next_btn, LV_OPA_0, LV_STATE_FOCUSED);
    lv_obj_set_style_border_opa(ui->next_btn, LV_OPA_0, LV_STATE_FOCUSED);
    lv_obj_set_style_outline_opa(ui->next_btn, LV_OPA_0, LV_STATE_FOCUSED);
    // 添加点击事件回调函数
